...
with engine.begin() as conn:
    if crud_action == "View":
        st.subheader(f"📋 Viewing Table: {crud_table.title()}")
        df = pd.read_sql_query(f'SELECT * FROM "{crud_table}"', conn)
        st.dataframe(df)

    elif crud_action == "Insert":
        st.subheader(f"➕ Insert into {crud_table.title()}")
        cols = pd.read_sql_query(f'SELECT * FROM "{crud_table}" LIMIT 0', conn).columns
        inputs = {col: st.text_input(col) for col in cols if col.lower() not in ['claim_id', 'food_id', 'provider_id', 'receiver_id']}
        if st.button("Insert Record"):
            try:
                col_names = ", ".join([f'"{col}"' for col in inputs.keys()])
                placeholders = ", ".join([f":{col}" for col in inputs.keys()])
                query = text(f'INSERT INTO "{crud_table}" ({col_names}) VALUES ({placeholders})')
        ...
        st.subheader(f"✏️ Update a Record in {crud_table.title()}")
        key_col = pd.read_sql_query(f'SELECT * FROM "{crud_table}" LIMIT 1', conn).columns[0]
        row_id = st.text_input(f"Enter {key_col} of the record to update")
        if row_id:
            df = pd.read_sql_query(f'SELECT * FROM "{crud_table}" WHERE "{key_col}" = %s', conn, params=(row_id,))
            if not df.empty:
                updates = {col: st.text_input(col, str(df[col][0])) for col in df.columns if col != key_col}
                if st.button("Update Record"):
                    try:
                        set_clause = ", ".join([f'"{k}" = :{k}' for k in updates.keys()])
                        updates["id"] = row_id
                        query = text(f'UPDATE "{crud_table}" SET {set_clause} WHERE "{key_col}" = :id')
        ...
        st.subheader(f"🗑️ Delete from {crud_table.title()}")
        key_col = pd.read_sql_query(f'SELECT * FROM "{crud_table}" LIMIT 1', conn).columns[0]
        row_id = st.text_input(f"Enter {key_col} to delete")
        if st.button("Delete Record"):
            try:
                conn.execute(text(f'DELETE FROM "{crud_table}" WHERE "{key_col}" = :id'), {"id": row_id})
